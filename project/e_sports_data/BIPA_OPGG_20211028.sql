...
# 3. summonerId = 25170222인 플레이어가 플레이한 게임 정보를 10개만 출력하는 쿼리 작성
# 출력 컬럼: 게임id, 챔피언id, 포지션, 승패
EXPLAIN
SELECT gameId, championId, position, result
FROM p_opGameStats
WHERE summonerId = 25170222
LIMIT 10;
...
# 출력 컬럼: 티어, 랭크, 유저 수(userCount)
# 높은 티어(챌린저>그랜드마스터>...), 높은 랭크(1>2>3>4)가 위에 출력되게 하세요.
# HAVING을 사용하는 쿼리, HAVING을 사용하지 않는 쿼리를 둘 중 아무거나 작성하세요.
...
# 9. 8번 문제에 나온 10명의 주말(2021년 10월 23일~2021년 10월 24일) 승률을 출력하는 쿼리 작성
# 출력 컬럼: 소환사id, 닉네임, 판 수(gameCount), 승률(Winrate)
# 다시하기 게임 제외합니다.
EXPLAIN
SELECT STRAIGHT_JOIN l.summonerId, name, COUNT(gameId) AS gameCount, SUM(result = 'WIN')/COUNT(gameId) AS Winrate
FROM opSummonerLeague l
LEFT JOIN opSummoner s
ON l.summonerId = s.id
...
AND `rank` = 1
AND result != 'UNKNOWN'
AND createDate >= '2021-10-23'
AND createDate < '2021-10-25'
...
# 다시하기 게임 제외합니다.
EXPLAIN
SELECT l.summonerId, name, IFNULL(SUM(dragonKills)/COUNT(o.gameId),0) AS AvgDragonKills
FROM opSummonerLeague l
...
# 12. 2021년 10월 23일 솔로 랭크 게임에서 각 챔피언의 픽 횟수, 승률, KDA 출력
...
INNER JOIN opGame o
ON p.gameId = o.gameId
WHERE o.createDate >= '2021-10-23'
AND o.createDate < '2021-10-24'
AND result != 'UNKNOWN'
AND subType = 420
